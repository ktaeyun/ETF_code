...
분산은 완벽히 맞으나 실제 괴리율은 뾰족하고 꼬리가 두꺼운데(첨도 2.15) Gaussian 혁신항을 쓰는
OU는 사실상 정규(0.13)를 낸다. PIT-KS도 GAP이 4개 중 최저 p(0.263)로 같은 방향을 가리키나
기각까지는 가지 않는다. 처방 후보: OU 혁신항을 Student-t로 (NAV가 이미 ARIMA-GARCH-t를 쓰므로 일관).
...
4. ~~**NAV·Combined 과대분산**~~ — **해소(오진이었음).** 근거였던 `mean_proximity ≈ -381.8`,
   ...
   **대신 GAP의 분포 모양 불일치가 새 미결 항목이다** (초과첨도 2.147 vs 0.128, 2.4 참조).
