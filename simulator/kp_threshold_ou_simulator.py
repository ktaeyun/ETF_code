...
def fit_threshold_ou(
    ...
    btc_kr = np.asarray(bitcoin_kr).flatten()
    ...
    return {
        ...
        'bitcoin_kr_mean': float(np.mean(btc_kr)),
        'bitcoin_kr_std': float(np.std(btc_kr)),
    }
...
def select_optimal_threshold(
    ...
    if threshold_candidates is None:
        ...
        threshold_candidates = np.array([
            np.percentile(abs_kp, 70),
            np.percentile(abs_kp, 80),
            np.percentile(abs_kp, 85),
            np.percentile(abs_kp, 90),
        ])
...
class KPThresholdOUSimulator:
    ...
    def __init__(self, fit_result: Dict, clip: float = 3.0):
        ...
        self.bitcoin_kr_mean = fit_result['bitcoin_kr_mean']
        self.bitcoin_kr_std = fit_result['bitcoin_kr_std']
        self.clip = clip
    ...
    def simulate_kp(
        ...
        if bitcoin_kr_future is None:
            ...
            btc_kr = np.asarray(bitcoin_kr_future).flatten()
            if len(btc_kr) < T:
                btc_kr = np.pad(btc_kr, (0, T - len(btc_kr)), mode='edge')
            else:
                btc_kr = btc_kr[:T]
